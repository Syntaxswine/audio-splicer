...
// Matches files this tool itself produced for a given mix name ("mix.ogg",
// "mix-2.mp3", ...) so rendering into the input folder never feeds earlier
// outputs back into the next mix.
export function mixArtifactRegex(baseName) {
  const esc = baseName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`^${esc}(-\\d+)?\\.(${Object.keys(ENCODERS).map(e => e.slice(1)).join('|')})$`, 'i');
}
...
// ---------------------------------------------------------------------------
// Time helpers shared by the CLI and the app.

export function parseLength(s) {
  if (!s) return null;
  const hms = String(s).match(/^(?:(\d+):)?(\d+):(\d{1,2}(?:\.\d+)?)$/);
  if (hms) {
    const [, h, m, sec] = hms;
    return (h ? +h * 3600 : 0) + +m * 60 + +sec;
  }
  const units = String(s).match(/^(?:(\d+(?:\.\d+)?)h)?(?:(\d+(?:\.\d+)?)m)?(?:(\d+(?:\.\d+)?)s)?$/);
